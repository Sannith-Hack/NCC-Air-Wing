...
const Profile = () => {
  ...
  const fetchStudentData = async () => {
    ...
    if (studentError) {
      toast.error("Database Error", { description: `Could not fetch profile: ${studentError.message}` });
  ...
  const handleStudentSubmit = async (e: React.FormEvent) => {
    ...
    if (studentData) {
      ...
      else { toast.success("Success", { description: "Profile updated successfully" }); await fetchStudentData(); }
    } else {
      const { error } = await supabase.from("students").insert([{ ...studentPayload, user_id: user.id }]);
      if (error) toast.error("Error", { description: error.message });
      else { toast.success("Success", { description: "Profile created! You can now add NCC & Experience details." }); await fetchStudentData(); }
    }
    setDataLoading(false);
  };
  ...
  const handleNccSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (nccDetails.length >= 10) { toast.error("Limit Reached", { description: "You cannot add more than 10 NCC detail records." }); return; }
    if (!studentData) { toast.error("Error", { description: "Please save your student details first" }); return; }
    ...
    if (error) { toast.error("Error", { description: error.message }); }
    else { toast.success("Success", { description: "NCC details added successfully" }); setNccForm({ ncc_wing: "air", regimental_number: "", enrollment_date: "", cadet_rank: "", my_ncc_certification: "N/D", camps_attended: "", awards_received_in_national_camp: "" }); await fetchStudentData(); }
  };
  
  const handleNccDelete = async (nccId: string) => {
    const { error } = await supabase.from("ncc_details").delete().eq("ncc_id", nccId);
    if (error) { toast.error("Error", { description: error.message }); }
    else { toast.success("Success", { description: "NCC record deleted successfully" }); await fetchStudentData(); }
  };
  const handleNccUpdate = async (nccId: string, updatedData: any) => {
    const { error } = await supabase.from("ncc_details").update(updatedData).eq("ncc_id", nccId);
    if (error) { toast.error("Error", { description: error.message }); }
    else { toast.success("Success", { description: "NCC record updated successfully" }); await fetchStudentData(); }
  };

  const handleExpSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (experiences.length >= 10) { toast.error("Limit Reached", { description: "You cannot add more than 10 experience records." }); return; }
    if (!studentData) { toast.error("Error", { description: "Please save your personal details first." }); return; }
    ...
    if (error) { toast.error("Error", { description: error.message }); }
    else { toast.success("Success", { description: "Experience added successfully" }); setExpForm({ experience: "internship", company_name: "", role: "", start_date: "", end_date: "" }); await fetchStudentData(); }
  };
  ...
  const handleExpDelete = async (experienceId: string) => {
    ...
    if (error) { toast.error("Error", { description: error.message }); }
    else { toast.success("Success", { description: "Experience record deleted successfully" }); await fetchStudentData(); }
  };
  const handleExpUpdate = async (experienceId: string, updatedData: any) => {
    const { error } = await supabase.from("placements_internships").update(updatedData).eq("experience_id", experienceId);
    if (error) { toast.error("Error", { description: error.message }); }
    else { toast.success("Success", { description: "Experience record updated successfully" }); await fetchStudentData(); }
  };
