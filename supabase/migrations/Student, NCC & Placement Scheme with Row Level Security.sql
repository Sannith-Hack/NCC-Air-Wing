-- Create enum types
CREATE TYPE ncc_wing_type AS ENUM ('air');
CREATE TYPE experience_type AS ENUM ('placement', 'internship');
-- Create user roles table for admin access
CREATE TYPE app_role AS ENUM ('admin', 'student');
CREATE TYPE ncc_certification AS ENUM ('A', 'B','C','Other','N/D');

-- Create Students table
CREATE TABLE Students (
    student_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL UNIQUE,
    role app_role NOT NULL DEFAULT 'student', -- Default value set to 'student'
